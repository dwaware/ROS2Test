...
namespace ROS2
{
    ...
    public class ROS2ListenerExample : MonoBehaviour
    {
        ...
        public SphereController mySphereController;
        ...
        void Update()
        {
            if (ros2Node == null && ros2Unity.Ok())
            {
                ...
                string_sub = ros2Node.CreateSubscription<std_msgs.msg.String>(
                  "string", msg => Debug.Log("Received string: [" + msg.Data + "]"));
                bool_sub = ros2Node.CreateSubscription<std_msgs.msg.Bool>(
                  "bool", msg => Debug.Log("Received bool: [" + msg.Data + "]"));
                float_sub = ros2Node.CreateSubscription<std_msgs.msg.Float32>(
                  "float", msg => Debug.Log("Received float: [" + msg.Data + "]"));
                key_value_sub = ros2Node.CreateSubscription<diagnostic_msgs.msg.KeyValue>(
                  "keyvalue", msg => Debug.Log("Received keyvalue: [ Key:" + msg.Key + ", Value: " + msg.Value + "]"));

                point_sub = ros2Node.CreateSubscription<geometry_msgs.msg.Point>(

                  "point", msg =>
                  {
                      Debug.Log("Received point: [ x: " + msg.X + ", y: " + msg.Y + ", z: " + msg.Z + "]");
                      Vector3 myVector = new Vector3((float)msg.X, (float)msg.Y, (float)msg.Z);
                      mySphereController.SetTarget(myVector);
                  });
            }
        }
    }
